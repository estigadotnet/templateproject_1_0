...
class HttpUpload
{
	...
	public function uploadFile()
	{
		...
		if (IsApi() && Post("addopt") != "1") {
			if (($name = Post($this->Parent->Name)) !== NULL) {
				if ($this->isFileToken($name)) { // File token
					$this->KeepFile = FALSE;
					$this->FileToken = $name;
					$this->FileName = $this->getUploadedFileName($this->FileToken);
				} else { // New file name (for removing old files)
					RenderUploadField($this->Parent, $this->Index); // Set up temp folder
					$this->KeepFile = FALSE;
					$this->FileName = $name;
				}
			} else {
	...
	/**
	 * Check if $name is file token
	 *
	 * @param string $name File token to locate the uploaded temp path
	 * @return boolean Return if name is file token
	 */
	public function isFileToken($name)
	{
		if (EmptyValue($name))
			return FALSE;
		$path = UploadTempPath($name, $this->Index);
		return @is_dir($path);
	}
